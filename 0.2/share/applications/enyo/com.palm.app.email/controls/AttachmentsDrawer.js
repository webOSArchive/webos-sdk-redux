...
enyo.kind({
	...
	handleContextItem: function(target) {
		...
		switch(action) {
			case "OPEN": this.doOpen(); break;
		 	case "SAVE": this.doSaveSingle(); break;
		 	case "SAVE ALL": this.doSaveAll(); break;
		 	default: console.error("unknown attachment menu action");
		}
	},
	...
	doSaveAll: function() {	
		...
		attachments.forEach(function(attch) {
			...
			if (attch.path) {
				...
				Attachment.saveAttachment(attch.path, fileName, function() {console.log("### Saved attachment."); });
			} else {
	...
	doSaveSingle: function() {
		...
		if (this.targetPart.path) {
			...
			Attachment.saveAttachment(this.targetPart.path, fileName, function() {console.log("### Saved attachment."); });
		} else {
	...
	getAttachmentItem: function(inSender, inIndex) {
		...
		if (attachmentPart) {
			...
			if(this.composeMode) {
				...
				return [{kind: "SwipeableItem", className:"compose-attachment-overide", onConfirm: "handleSwipeDelete", attachmentIndex: inIndex, components: [attachmentItem]}];
			} else {
