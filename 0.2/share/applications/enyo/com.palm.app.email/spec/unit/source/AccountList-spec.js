...
var TEST_CALL_SERVICE = function(uri, args, callback) {
	...
	if (resultsContainer.results) {
		...
		setTimeout(function() {callback(resultsContainer);}, 300);
	}
};
...
describe("AcountList Load", function() {
	...
	it('loads accounts correctly', function() {
		runs(function () {
			accountList = new EmailApp.AccountList(function() { ready = true;}, forcefulFlag);
		});
		waitsFor(function () {
			return ready;
	...
	it('retrieves accounts correctly', function() {
		...
		runs(function () {
			...
			accts.forEach(function(acct) { console.log("#@#$!@$!$2 Here's an account"); console.dir(acct); });
