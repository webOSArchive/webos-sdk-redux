...
Email._getAdjacentMessage = function (email, isNext, isDescending, baseQuery, callback) {
	...
	var whereClause = baseQuery && baseQuery.where || [{prop:"flags.visible", op:"=", val:true}, {prop:"folderId", op:"=", val:email.folderId}];
	...
	var ineq = isNext ? "<" : ">";
	var locWhere = [{prop: orderBy, op:ineq, val:Email._lookupProperty(email,orderBy), collate:"primary"}];
...
enyo.kind({
	...
	_getUpdates: function() {
		this._subscription = EmailApp.Util.callService('palm://com.palm.db/find',
			{
				query: {from: Email.KIND, where: [{prop: "_id", op: "=", val: this.data._id}]},
				watch: true,
				subscribe: true
			},
