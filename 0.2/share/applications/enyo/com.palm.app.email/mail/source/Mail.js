...
//		{kind: "Divider", caption: "TODAY", showing: false, className: "today-divider"},
		...
		{kind: "Toolbar", style:"z-index:1;", name:"mailToolbar", className:"mail-command enyo-toolbar-light", components: [  //changed the z-index to 1 to prevent the command menu from showing over the 3rd panel
	...
	*/
	...
	getListQuery: function(inQuery) {
		...
		if (folderId === Folder.kAllInboxesFolderID) {
			query.where = [{ prop: "flags.visible", op: "=", val: true }, { prop: "withinInbox", op: "=", val: true }];
		...
		*/
			...
			query.where = [{ prop: "flags.visible", op: "=", val: true }, { prop: "flags.flagged", op: "=", val: true }];
			query.orderBy = "sortKeys.allFlagged";
		} else if (folderId) {
			query.where = [{ prop: "flags.visible", op: "=", val: true }, { prop: "folderId", op: "=", val: folderId }];
	...
	listQuery: function(inSender, inQuery) {
		...
		if (query) {
			...
			if (enyo.application.dbServiceHook) { enyo.application.dbServiceHook(this.$.emailFind); }
...
//		this.maybeActivateOnRefresh();
	...
	aboutClick: function() {
		if (window.PalmSystem) {
			EmailApp.Util.callService('palm://com.palm.db/find',
				{query: {from:"com.palm.email:1", where:[{prop:"folderId", op:"=", val:this.folder._id}], limit:0}, count:true},
	...
	emptyTrashConfirmClick: function() {
		...
		if (this.isTrashFolder(this.folder)) {
			...
			query.where = [{ prop: "folderId", op: "=", val: this.folder._id }];
    ...
    showAboutFolder: function(response) {
		...
		if (!account || this.folder._id === Folder.kFavoriteFoldersAccountID) {
			...
			var info = {
				account: account,
				components: [
					{content: lastUpdatedStr}, // TODO need non-escaped HTML once escaping is enabled
