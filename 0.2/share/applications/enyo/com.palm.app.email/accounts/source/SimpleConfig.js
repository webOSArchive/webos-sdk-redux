			...
			{kind:"Control", className:"box-center", components: [ 
				{
					...
					components : [{name : "email", kind : "Input", inputType: "email", value : "", spellcheck: false, oninput: "checkUsername", onkeydown: "checkEnter", onblur: "checkUsername", autoCapitalize: "lowercase", autocorrect:false, className: "slartibartfast"}]
				},
				{
					...
					className:"accounts-group",
					showing: false,
					components : [{name : "username", kind : "Input", inputType: "email", value : "", spellcheck: false, oninput: "checkForm", onkeydown: "checkEnter", autoCapitalize: "lowercase", autocorrect:false, className: "slartibartfast"}]
				},
				{
					kind : "RowGroup",
					caption : $L("PASSWORD"),
					className:"accounts-group",
					components : [{name : "password", kind : "Input", inputType: "password", value : "", spellcheck: false, oninput: "checkForm", onkeydown: "checkEnter", autocorrect:false, onfocus:"checkPass", className: "slartibartfast"}]
				},
	...
	createError: function(err) {
		...
		if (!info.genericError && (!settings || settings.protocol !== AccountWizard.EAS)) {
			...
			if (false) { 
				...
				if (info.retry && false) {
					choices = [{caption: $L("Retry")}, {caption: $L("Cancel")}];
				} else {
					choices = [{caption: $L("OK")}];
				}
				...
				if (info.messageParams.errorMessage) {
					if (messageHtml.length > 0) { messageHtml += "<br><br>"; }
