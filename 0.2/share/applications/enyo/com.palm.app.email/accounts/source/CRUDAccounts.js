...
enyo.kind({
	...
	checkPasswords: function(onComplete) {
		...
		var parseIn = function(credentials) { parseCred("common", credentials); };
		var parseOut = function(credentials) { parseCred("smtp", credentials); };
	...
	loadAccountFromDb: function(account) {
		...
		this._mailQuery = EmailApp.Util.callService('palm://com.palm.db/find', {query: {from:"com.palm.mail.account:1", "where":[{prop: "accountId", "op": "=", "val": ca.getId()}]}}, handleMail);
	},
	...
	duplicateAccountHandler: function(inSender, wizardResult) {
		...
		if (wizardResult.isDuplicateAccount) {
			...
			this.showError({
					...
					onChoose: function(value) { that.exitWizard(); }
				});
		} else {
	...
	showError: function(toDisplay) {
		...
		var i = 0, choices = toDisplay.choices || [{label:$L('OK'), value:'dismiss', type:'dismiss'}], btn;
		// show needed fields
	...
	removeSecurityPolicies: function(accounts) {
		...
		EmailApp.Util.callService("palm://com.palm.db/del", {
				query: {
					...
					where: [{"prop":"isTemp","op":"=","val":true}]
				}
			}, function() { console.log("deleted sec policy");
							that.exitWizard();
						});
		}
	});
