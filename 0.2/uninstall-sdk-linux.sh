#!/bin/bash
#
# Uninstallation script for webOS SDK on Linux
#
...
if [ "$EUID" -ne 0 ]; then
    log_error "Please run with sudo: sudo ./uninstall-sdk-linux.sh"
    exit 1
fi

echo ""
echo "=========================================="
echo "  webOS SDK Uninstallation for Linux"
echo "=========================================="
echo ""
...
for path in "/opt/PalmSDK" "/usr/local/PalmSDK"; do
    if [ -d "$path" ]; then
...
echo ""
BACKUP_LOCATIONS=()
for base_path in "/opt" "/usr/local"; do
...
for bin_path in "/usr/local/bin" "/opt/nova/bin" "/usr/bin"; do
